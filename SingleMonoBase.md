...
- 大白话比喻：这是 Unity 官方独家定制的“专用配件包”。
    - 前两个工具箱（System开头）是微软提供的通用螺丝刀，不管你是做网站、做系统还是做软件都能用。   
...
|（）|方法名|
|空格 + 另一个名字|变量名|
|点 `.`|可能是任何东西|点前面是“谁的”，要看后面跟什么决定（后面跟括号是方法，后面跟名字是变量）|
public class SingleMonoBase<T>   后面是 <T> → 它是类名
protected virtual void Awake()    后面是 () → 它是方法名
public static T INSTANCE;        // 前面有 T（职位），空格后是它 → 它是变量名
